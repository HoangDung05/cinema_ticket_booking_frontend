...
function formatDiscountValue(v: VoucherDto) {
  if (v.discountType === 'PERCENT') {
    return `${Number(v.discountValue)}%`;
  }
  return `${Number(v.discountValue).toLocaleString('vi-VN')} ₫`;
}
...
export default function Vouchers() {
  ...
  const saveVoucher = async () => {
    if (!form.code.trim() || !form.startDate || !form.endDate) {
      window.alert('Nhập mã, ngày bắt đầu và ngày kết thúc.');
      return;
    }
    setSaving(true);
    try {
      ...
      console.error(e);
      window.alert('Lưu voucher thất bại (kiểm tra mã trùng hoặc dữ liệu).');
    } finally {
      setSaving(false);
    }
  };
  ...
  const deleteVoucher = async (id: number) => {
    ...
    try {
      ...
      console.error(e);
      window.alert('Xóa voucher thất bại.');
    }
  };
              ...
              <tbody className="text-sm divide-y divide-gray-100">
                {filteredRows.map((row) => (
                  <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4 font-semibold text-sky-600 whitespace-nowrap">{typeLabel(row.discountType)}</td>
                    <td className="p-4 text-gray-900 font-medium whitespace-nowrap">{formatDiscountValue(row)}</td>
